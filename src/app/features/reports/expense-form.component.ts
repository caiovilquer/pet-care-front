...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content><form id="expense-form" [formGroup]="form" (ngSubmit)="save()">
      ...
      <div class="row"><mat-form-field appearance="outline"><mat-label>Valor</mat-label><span matTextPrefix>R$&nbsp;</span><input matInput type="number" formControlName="amount" min="0.01" max="9999999999.99" step="0.01" required><mat-error>Informe um valor positivo</mat-error></mat-form-field>
        <mat-form-field appearance="outline"><mat-label>Moeda</mat-label><mat-select formControlName="currency"><mat-option value="BRL">BRL</mat-option></mat-select></mat-form-field></div>
  ...
  styles: [`
    ...
    form{display:grid;gap:4px;padding-top:8px}.row{display:grid;grid-template-columns:1fr 1fr;gap:12px}mat-form-field{width:100%}mat-dialog-actions{padding:12px 24px 20px}mat-spinner{display:inline-block;margin-right:8px}
    @media(max-width:520px){.row{grid-template-columns:1fr;gap:0}.head{padding-inline:16px}}
  `]
})
export class ExpenseFormComponent {
  ...
  constructor(
    ...
    this.form = fb.nonNullable.group({
      ...
      amount: [data.expense?.amount || 0, [Validators.required, Validators.min(.01), Validators.max(9999999999.99)]],
      currency: [data.expense?.currency || 'BRL', [Validators.required, Validators.pattern(/^[A-Z]{3}$/)]],
  ...
  save(): void {
    ...
    const request: ExpenseRequest = {
      ...
      currency: value.currency, occurredAt: occurred.toISOString(), notes: value.notes.trim() || null,
    };
    this.saving = true;
